...
/* 在dbvisualizer客户端执行存储过程和函数(方法)时,需要在存储过程或函数(方法)的最前面加上 --/  以及在最后面添加 /
--/
CREATE  PROCEDURE pro_doorder(IN i_goods_id BIGINT,IN i_mobile varchar(11),IN i_order_time TIMESTAMP ,OUT o_result INT(1))
BEGIN
	  DECLARE insert_count INT DEFAULT 0;
	    START TRANSACTION ;
	    INSERT IGNORE INTO  guiyang.order(mobile,goods_id,num,create_time)VALUES(i_mobile,i_goods_id,1,i_order_time);
	    SELECT ROW_COUNT() INTO insert_count;
	    IF(insert_count = 0)THEN
	     ROLLBACK ;
	     SET o_result=-1;
...
*/
/*
CURRENT_TIMESTAMP(库中添加当前时间戳)
 INSERT IGNORE INTO  guiyang.order(mobile,goods_id,num,create_time)VALUES('13831429380','1',1,CURRENT_TIMESTAMP);
*/